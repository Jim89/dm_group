...
This document has interactive elements and is best viewed using a modern web brower such as Mozilla Firefox or Google Chrome.
...
kable(tidy(model211), col.names = tidy_names, caption = "Table 1: Coefficients", digits = 4)
kable(glance(model211), col.names = toproper(glance(model211) %>% names()), caption = "Table 2: Regression Statistics", digits = 4)
```
...
```{r show_store_model}
kable(tidy(model212), col.names = tidy_names,caption = "Table 3: Coefficients", digits = 4)
kable(glance(model212), col.names = toproper(glance(model212) %>% names()), caption = "Table 4: Regression Statistics", digits = 4)
```
...
```{r show_mail_model}
kable(tidy(model213), col.names = tidy_names, caption = "Table 5: Coefficients",digits = 4)
kable(glance(model213), col.names = toproper(glance(model213) %>% names()),caption = "Table 6: Regression Statistics", digits = 4)
```
...
```{r show_phone_model}
kable(tidy(model214), col.names = tidy_names,caption = "Table 7: Coefficients", digits = 4)
kable(glance(model214), col.names = toproper(glance(model214) %>% names()),caption = "Table 8: Regression Statistics", digits = 4)
```
...
```{r, echo=FALSE}
kable(tidy(model224), col.names = tidy_names, caption = "Table 9: Coefficients", digits = 4)
kable(glance(model224), col.names = toproper(glance(model224) %>% names()),caption = "Table 10: Regression Statistics", digits = 4)
```
...
```{r}
kable(tidy(model221), col.names = tidy_names,caption = "Table 11: Coefficients", digits = 4)
kable(glance(model221), col.names = toproper(glance(model221) %>% names()), caption = "Table 12: Regression Statistics", digits = 4)
```
...
```{r}
kable(tidy(model223), col.names = tidy_names, caption = "Table 13: Coefficients", digits = 4)
kable(glance(model223), col.names = toproper(glance(model223) %>% names()), caption = "Table 14: Regression Statistics", digits = 4)
```
...
```{r}
kable(tidy(model222), col.names = tidy_names, caption = "Table 15: Coefficients",digits = 4)
kable(glance(model222), col.names = toproper(glance(model222) %>% names()), caption = "Table 16: Regression Statistics",digits = 4)
```
...
In the figure below, we present an example of the resulting graph structure. (Note that due to the size of the graph, not all nodes are shown). There is a clear structure to this graph. The majority of nodes are not connected, with most customers sending/receiving gifts to/from a very small number of other customers. Groups are small, with little connection between groups of customers, there are very few chains or cycles in the graph. There are, however, some users who have sent a huge number of gifts and therefore stand out in the network representation. (Note that the visualisatin is interactive - click and drag/mousewheel zoom to navigate the network).
...
kable(tidy_fit, col.names = tidy_names, digits = 3)
...
From this model, it is clear that out-degree centality does have a statistically and economically significant effect on total spend, when controlling for number of orders. The estiamtes are large and statistically significant, and the overall model has a good fit, explaing around 60% of the variation in the data. Therefore we have considered out-degree centrality to be a plausible dimension to segment customers on for marketing purposes, as explored in section C. 
...
kable(tidy(model), caption = "Table 1: Regression Coefficients", digits = 3, col.names = tidy_names)
kable(glance(model), caption = "Table 2: Regression Statistics", digits = 3, col.names = toproper(glance(model) %>% names()))
```

Whilst the coefficients of the model are typically significant, the R^2^ value is low.  Even though the coefficents reveal the presence of a trend, such a low R^2^ indicates that having children is only a small factor in determining how much a customer will spend. 
...
kable(tidy(model2), caption = "Table 3: Coefficients", digits = 3, col_names = tidy_names)
kable(glance(model2), caption = "Table 4: Regression Statistics", digits =3, col.names = toproper(glance(model2) %>% names()))
```
...
```{r kin_email_dist, fig.cap = "Figure 13: Distribution of number of emails and catalogues sent to customers", fig.align="center"}
...
We then calcuted the RFM segment for each customer in each of these four data sets. Recency was calculted using the order that was closest to the end of the period for each data set (2006-06-30 for the first half of the data, and 2008-01-01 for the second period). Frequency was estimated via a simple count of orders that were made in the period. Monetary was the total dollars spent by each customer in the period.
...
We first assessed the range of response rates for both emails and catalogues, presented in figures 14 and 15.

```{r email_resp, fig.cap = "Figure 14: The range of email response rates", fig.align="center"}
email_1_rfm_stats %>% 
...
```{r cat_resp, fig.cap = "Figure 15: The range of catalogue response rates", fig.align="center"}
cat_1_rfm_stats %>% 
    ggplot(aes(x = response_rate)) +
...
email_comp %>% 
    kable(col.names = c("RFM", "Precited Response", "Actual Response", "Difference"),
          digits = 3,
          caption = "Table : Difference in predicted and actual response rates for email contacts")
```
...
cat_comp %>% 
    kable(col.names = c("RFM", "Precited Response", "Actual Response", "Difference"),
          digits = 3,
          caption = "Table : Difference in predicted and actual response rates for catalogue contacts")
```
...
email_comp2 %>% 
    kable(col.names = c("RFM", "Predicted Response", "Actual Response", "In Strategy"),
          digits = 3,
          caption = "Table : Comparison of email strategies between first and second half of data")
```
...
cat_comp2 %>% 
    kable(col.names = c("RFM", "Predicted Response", "Actual Response", "In Strategy"),
          digits = 3,
          caption = "Table : Comparison of catalogue strategies between first and second half of data")
```

As can be seen in the tables, our strategies are shown to be valid based on applying the same strategies devleoped from the first period to the validation data from the second period. For emails, `r scales::percent(mean(email_comp2$did_mail))` of the segments that should have been chosen in the validation data were chosen in the first period. For catalogues, the same figure is `r scales::percent(mean(cat_comp2$did_mail))`.

## Spend comparisons {.tabset}

Finally, we can compare the spend of each segment in the data from the first and second periods. For emails, our strategy would have resulted in contacting all of the top 10 RFM segments when ranked by total spend of customers in each segment. For catalogues we mailed 80% of the top 5 RFM segments when ranked by total spend of customers in each segment. This helps to further demonstrate the validity of our approach.
...
We also sought to extend our marketing strategy using another dimension in the RFM model. Having explored several possiblities in section A, we noted that only the level of gift giving (as measure by the out-degree centrality on the gift giving network