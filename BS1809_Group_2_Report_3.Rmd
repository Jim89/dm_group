...
We also considered a simple logistic regression model in which _any_ type of customer response was modelled as a function of position and price in order to understand their effect on either clicking _or_ booking. In doing so we also included the star rating for each hotel, and indication of whether the hotel was on promotion or not, and the length of stay searched for. This helped us to isolate the true effects of price and position. The results from this model are displayed in table two.

```{r simple_model, cache = T}
data <- data %>% 
        mutate(response = ifelse(click_bool == 1 | booking_bool == 1, 1, 0))

simple_fit <- glm(response ~ position + price_usd + prop_starrating + srch_length_of_stay, 
...
It is seen that both position and price have a statistically significant effect on the log-odds (and therefore the likelihood) of a consumer responding. However, the effect of position is much larger than for price, again approximately an order of magnitude larger.
